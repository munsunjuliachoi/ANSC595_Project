...
evenness = read_qza("~/Desktop/core-metrics-results/evenness_vector.qza")
...
observed_features = read_qza("~/Desktop/core-metrics-results/observed_features_vector.qza")
...
shannon = read_qza("~/Desktop/core-metrics-results/shannon_vector.qza")
...
faith_pd = read_qza("~/Desktop/core-metrics-results/faith_pd_vector.qza")
...
colnames(meta)[3] = "bodysite"
aov.evenness.bodysite = aov(pielou_evenness ~ bodysite, data=meta)
...
TukeyHSD(aov.evenness.bodysite)
...
levels(meta$bodysite)
#Re-order the groups because the default is 1yr-2w-8w
meta$bodysite.ord = factor(meta$bodysite, c("left palm", "right palm", "gut", "tongue"))
levels(meta$bodysite.ord)

#Plot
boxplot(pielou_evenness ~ bodysite.ord, data=meta, ylab="Peilou evenness")

evenness <- ggplot(meta, aes(x = bodysite.ord, y = pielou_evenness)) + 
  geom_boxplot(aes(color = bodysite.ord)) + 
  ylim(c(0.5,1)) +
...
kruskal.test(faith_pd ~ bodysite.ord, data=meta)
...
pairwise.wilcox.test(meta$faith_pd, meta$bodysite.ord, p.adjust.method="BH")
